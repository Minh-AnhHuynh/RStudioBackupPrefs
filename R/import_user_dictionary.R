...
import_user_dictionary <- function(
    ...
    user_dic_path = get_user_dictionary_path()
) {
  ...
  user_file <- glue::glue({local_user_dict_path}, "user_dictionary/user_dictionary")
  fs::file_copy(user_file, user_dic_path, overwrite = TRUE)
